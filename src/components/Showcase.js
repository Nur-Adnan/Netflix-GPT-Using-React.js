...
const Showcase = ({ data }) => {
  ...
  const { id, title, overview, backdrop_path, poster_path, genre_ids } =
    data?.info;
  ...
  return (
    <div
      className={`showcase md:h-screen mt-[-70px] xl:h-auto xl:aspect-video bg-gradient-to-b from-slate-700 xl:mt-[-180pxL] transition-all`}
    >
      <div className="w-full h-full w-96F h-96F relative">
        ...
        <div className="overlay h-full w-full z-10 relative">
          ...
          <div
            className="hidden md:flex bg-rrr w-full h-full items-end pb-[20%]"
            style={{
              background:
                "linear-gradient(77deg,rgba(0,0,0,.8),transparent 85%)",
            }}
          >
            <div className="px-4 md:px-12 w-full xl:w-1/2 text-white pt-[0px]">
              <h1 className="text-4xl md:text-5xl lg:text-6xl mb-2 line-clamp-2 font-bold">
                {title}
              </h1>
              <p className="text-sm md:text-base line-clamp-2 md:line-clamp-2 xl:line-clamp-3">
                {overview}
              </p>
              <div className="action flex gap-3 mt-4">
                <Link
                  to={`${PAGE.WATCH}/${contentId}`}
                  className="px-4 md:px-6 py-[5px] font-bold text-md bg-white text-black rounded-[4px] flex items-center justify-center gap-2"
                >
                ...
                <button
                  className="px-4 md:px-6 py-[5px] font-bold text-md rounded-[4px] flex items-center justify-center gap-2"
            ...
            <div className="px-4 md:px-12 w-full xl:w-1/2 text-white pt-[0px]">
              <div className="action flex gap-3 mt-4 justify-end">
                ...
                >
                  {isMuted ? (
                    <VolumeOffIcon style={{ fontSize: "28px" }} />
                  ) : (
                    <VolumeMuteIcon style={{ fontSize: "28px" }} />
                  )}
                </button>
              </div>
            </div>
          ...
          {/* For Mobile Onlye */}
          <div
            className="md:hidden bg-rrr pb-16 md:pb-0 backdrop-blur-xl md:backdrop-blur-0 w-full h-full flex items-centerF"
            style={{
              background:
                "linear-gradient(180deg, rgba(0, 0, 0, 0.01) 60%, #141414 100%)",
            }}
          >
            <div className="px-4 md:px-12 w-full text-white">
              <div className="relative h-[415px] max-w-80 m-auto  mt-[140px] rounded-lg overflow-hidden shadow-md">
                <div className="mobile-cover w-full h-full mt-[-5%]">
                  <img
                    src={`${TMDB_CDN_URL}/w500${poster_path}`}
                ...
                >
                  <h1 className="text-2xl font-bold md:text-5xl lg:text-6xl mb-2">
                    {title}
                  </h1>
                  ...
                  <div className="action flex items-center justify-center gap-3 mt-4">
                    <Link
                      to={`${PAGE.WATCH}/${contentId}`}
                      className="px-4 md:px-6 py-[5px] font-bold text-sm bg-white text-black rounded-[4px] flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      className="px-4 md:px-6 py-[5px] font-bold text-sm rounded-[4px] flex items-center justify-center gap-2"
                      style={{ background: "rgba(109, 109, 110, 0.7)" }}
                    >
