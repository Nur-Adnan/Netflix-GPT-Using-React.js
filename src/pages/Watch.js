...
const Watch = () => {
  ...
  if (videos?.results?.length <= 0)
    return <VideoNotFound data={{ original_title, backdrop_path }} />;
  ...
  return (
    <div className="w-screen h-screen">
      <div className="w-full h-full">
        <img
          src={`${TMDB_CDN_URL}/w500${backdrop_path}`}
          alt={original_title}
      ...
      <div className="absolute w-full h-full bg-black/70 top-0 bottom-0 left-0 right-0 flex flex-col justify-center items-center">
        ...
        <p className=" text-gray-300 text-2xl mb-4">
          Video not available for <b>{original_title}</b>
        </p>
        <Link
          to={PAGE.BROWSE}
          className="flex items-center justify-center px-6 py-3 mt-4 bg-[#e50914] text-white font-bold text-lg rounded-[4px] hover:bg-[#f40612] disabled:bg-red-800 transition-all duration-300 w-auto"
        >
          <MdHome className="text-[24px] mr-2" /> {/* Home icon */}
          <span className="icon-fill text-[24px]">Go to Homepage</span>
        </Link>
      </div>
    </div>
  );
